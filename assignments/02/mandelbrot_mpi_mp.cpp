...
void calcMandelbrot(Image &image, int size_x, int size_y) {
	...
	int end_row = (rank == num_ranks - 1) ? size_y : start_row + rows_per_rank;

	Image image_local(num_channels * size_x * rows_per_rank);
	...
	for (int pixel_y = start_row; pixel_y < end_row; pixel_y++) {
		...
		for (int pixel_x = 0; pixel_x < size_x; pixel_x++) {
			...
			int channel = 0;
			image_local[index(pixel_y - start_row, pixel_x, rows_per_rank, size_x, channel++)] = (uint8_t)(red * UINT8_MAX);
			image_local[index(pixel_y - start_row, pixel_x, rows_per_rank, size_x, channel++)] = (uint8_t)(green * UINT8_MAX);
			image_local[index(pixel_y - start_row, pixel_x, rows_per_rank, size_x, channel++)] = (uint8_t)(blue * UINT8_MAX);
		}
	}

	MPI_Gather(image_local.data(), num_channels * size_x * rows_per_rank, MPI_UINT8_T, image.data(), num_channels * size_x * rows_per_rank, MPI_UINT8_T, 0, MPI_COMM_WORLD);
	
	auto time_end = std::chrono::high_resolution_clock::now();
	auto time_elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(time_end - time_start).count();
	
	std::cout << "Mandelbrot set calculation for rank " << rank << " of " << num_ranks << " in total, took: " << time_elapsed << " ms, for a picture of size: " << size_x << "x" << size_y << "." << std::endl;
